...
recrate = .01 #daily probability of recovery
exposerate = 0.375 # How many people a single person potentially infects per day -- can be calculated from R0 estimate if you divide R0 by infectious period
exposepd = 5 # incubation period

patnInf[which(patNames == 42010000)] = 1

pat_locator$control_date = rep(date("2015-06-01"),NPat)#c(rep(.05,10),rep(0,length(pat_locator$patIDs)-10))
pat_locator$control_date[which(pat_locator$patNames == 42010000)] = date("2015-01-23")
#c(rep(.05,10),rep(0,length(pat_locator$patIDs)-10))
...
###### Master function  ####
HPop_update = runSim(HPop,pat_locator,movement_data, date("2014-12-08"), date("2015-2-1") , recrate, exposerate,exposepd,relative_movement = 1)
...
epidemic_curve = HPop_update$epidemic_curve
all_spread = HPop_update$all_spread
testdf = rbind(testdf,data.frame(num = sum(newHPop$nInf)))
}
...
for (i in 1:(as.numeric(date("2015-1-23")-date("2014-12-08")))){
...
saveGIF(trace.animate(plots),interval=.3,movie.name="test2.gif",ani.width=800,ani.height=800)
