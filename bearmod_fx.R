...
movementTimeStep = function(HPop, mobmat,day,control_df){
  ...
  if (length(which(control_df$date == day)) > 0){
    ...
    for (i in 1:dim(control_df_sub)[1]){
      ...
      if (control_df_sub$relative_move[i] < 1){
    #print(paste0("Instating travel ban in patch ", which(HPop$names == control_df_sub$from[i]) , "; relative movement ",control_df_sub$relative_move[i], " on day ", day  ))
      }
    }
  }
...
stopMovement = function(HPop,mobmat,current_date){
  stopping = which(HPop$controlled > 0)
    ...
    control_patches = HPop$patIDs[ctrl_pat]
